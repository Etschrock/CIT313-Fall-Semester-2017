...
<div id="wrapper">
    ...
    <?php
		...
		//had to delete an r from $arryProductArray so the function worked
		for ($x = 0; $x < count($aryProductArray); $x++) {
			...
			switch ($thisProduceItem->price_type) {
				...
				//added a } to close the switch
			}
					?>
					</div>

					<div><label for="quantity<?php echo $x ?>">Quantity Requested</label> <input
							id="quantity<?php echo $x ?>" name="quantity<?php echo $x ?>" type="text" size="10"/></div>
					</fieldset>
