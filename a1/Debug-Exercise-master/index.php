...
<div id="wrapper">
	...
	<div class="form-container">
		...
		<!-- Removed the echo because it was pushing that into the url-->
	<form action="<?php URL_ROOT ?>add_products_to_inventory.php" method="post">
		...
		for ($x = 0; $x <= 7; $x++) {
			...
			<fieldset>
				...
				<div><label for="productname<?php echo $x ?>">Product Name</label> <input
						id="productname<?php echo $x ?>" type="text" name="productname<?php echo $x ?>" value=""/></div>
				...
				<div><label for="productprice<?php echo $x ?>">Product Price</label> $<input
						id="productprice<?php echo $x ?>" name="productprice<?php echo $x ?>" type="text" size="10"/>
				</div>
			</fieldset>

			<?php

			//changed ) to } to end the function
		}
	?>
